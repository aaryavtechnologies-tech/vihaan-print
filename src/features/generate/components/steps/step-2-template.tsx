...
export function Step2Template() {
  const { filters, templateId, setTemplateId, nextStep, prevStep } = useGenerationStore();
  const [templates, setTemplates] = useState<any[]>([]);

  useEffect(() => {
    getSetupData().then(data => {
      // Filter templates belonging to the selected school or global ones (schoolId = null)
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-300">
      <div className="text-center space-y-2">
        ...
        <p className="text-slate-500">Choose the ID Card design for this generation batch.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        {templates.length === 0 && (
          <div className="col-span-full py-12 text-center text-slate-500 border-2 border-dashed border-slate-200 rounded-2xl">
            No templates found for this school. Please create one in the Template Editor.
          </div>
        )}
      </div>
