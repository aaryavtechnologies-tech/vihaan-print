...
export function Step1School() {
  const { filters, setFilters, nextStep } = usePrintStore();
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-300">
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">Select School</h2>
        <p className="text-slate-500">Choose the school to fetch generated ID cards for printing.</p>
      </div>
      ...
      <div className="space-y-6 bg-slate-50 p-8 rounded-3xl border border-slate-100">
        <div className="space-y-3">
          ...
          <Select value={filters.schoolId || undefined} onValueChange={(val) => setFilters({ schoolId: val || "" })}>
